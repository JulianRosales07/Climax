...
export const getWeeklyTopProducts = () => {
  const sales = getSales().filter(sale => {
    const saleDate = new Date(sale.date);
    const weekAgo = subDays(startOfDay(new Date()), 7);
    return saleDate >= weekAgo && sale.status === 'paid';
  });

  return getTopProductsFromSales(sales);
};

export const getMonthlyTopProducts = () => {
  const sales = getSales().filter(sale => {
    const saleDate = new Date(sale.date);
    const monthAgo = subMonths(startOfDay(new Date()), 1);
    return saleDate >= monthAgo && sale.status === 'paid';
  });

  return getTopProductsFromSales(sales);
};

const getTopProductsFromSales = (sales: Sale[]) => {
  const productSales = sales.reduce((acc, sale) => {
    if (!acc[sale.productId]) {
      acc[sale.productId] = { quantity: 0, totalSales: 0 };
    }
